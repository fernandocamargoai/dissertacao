...
Além disso, podemos notar que o claro vencedor nesse período de acordo com esse indicador foi o algoritmo MOCell, seguido pelo NSGA-II. Vale lembrar que o \textit{I}$^{1}_{\epsilon+}$ mede apenas a convergência desses algoritmos em relação a frente de pareto de referência. Ou seja, mede o quão próximo seus resultados ficaram dessa referência, sendo melhor um resultado menor.
...
Então, com o estudo dessas três métricas, podemos concluir que os algoritmos evolucionários obtêm melhor convergência da frente de pareto, enquanto os de enxame de partículas obtêm soluções mais distribuídas. Porém, o resultado geral é que os algoritmos evolucionários se mostraram mais adequados para esse problema nesse período.
...
\begin{sidewaysfigure}[h]
  \centering
  \caption{Frentes de Pareto das Médias do Indicador \textit{I}$^{1}_{\epsilon+}$ do primeiro período}
...
\begin{sidewaysfigure}[h]
  \centering
  \caption{Frentes de Pareto das Médias do Indicador $\Delta$ do primeiro período}
...
\begin{sidewaysfigure}[h]
  \centering
  \caption{Frentes de Pareto das Médias do Indicador HV do primeiro período}
...
\begin{sidewaysfigure}[h]
  \centering
  \caption{Frentes de Pareto das Médias do Indicador \textit{I}$^{1}_{\epsilon+}$ do segundo período}
...
\begin{sidewaysfigure}[h]
  \centering
  \caption{Frentes de Pareto das Médias do Indicador $\Delta$ do segundo período}
...
\begin{sidewaysfigure}[h]
  \centering
  \caption{Frentes de Pareto das Médias do Indicador HV do segundo período}
  \resizebox{\columnwidth}{!}{\input{./fig/Frente-de-Pareto_HV_2.pdf_tex}}
  \label{fig:paretoHV2}
\end{sidewaysfigure}

A primeira análise que podemos observar nessas figuras é que elas deixam claro a maior facilidade na otimização para esse segundo período. A própria Frente de Pareto de referência obteve valores maiores de 82\% de armazenamento final. Inclusive, muitos resultados se encontraram muito próximos de 100\% de armazenamento. Mesmo na Figura \ref{fig:paretoSPREAD2}, que avalia a distribuição das soluções, os valores não foram mais baixos do que 70\%. Isso comprova que realmente os algoritmos tiveram maior facilidade na solução desse problema.
...
Por fim, temos o último Indicador: Hipervolume. A Tabela \ref{table:hvPeriodo3} mostra os resultados dele. E como podemos ver, apenas dois algoritmos tiveram valor atribuído. O que significa que todos os outros tiveram sua extensão maiores do que a Frente de Pareto de referência. Porém, apesar da falta de valores, essa tabela reforça o fato do NSGA-II obter resultados que dominam os do MOCell. Diferente do segundo período, isso foi bem demonstrado tanto no primeiro quanto no terceiro período através do HV.
...
\begin{sidewaysfigure}[h]
  \centering
  \caption{Frentes de Pareto das Médias do Indicador $\Delta$ do terceiro período}
...
\begin{sidewaysfigure}[h]
  \centering
  \caption{Frentes de Pareto das Médias do Indicador HV do terceiro período}
